...
A ring is a sparse network (few arcs) such as trees, but it's symmetric: each entity has two neighbors.
\clearpage
...
In general, for a \textit{k}-bit communicator, we have that we use \textit{k} bits to send the message and the time complexity is $\mathcal{O}(k\cdot X^{\frac{1}{k}})$. (the teacher says so, actually it is $\mathcal{O}(k\cdot X^{\frac{1}{k-1}})$)
...
Combines characteristics of pure and hybrid P2P systems.
\begin{compactitem}
